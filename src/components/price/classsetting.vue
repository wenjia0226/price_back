    ...
    <!-- 面包屑导航区域 -->
     ...
     <el-card  style="margin: 20px 0">
      <el-row :gutter="20">
       ...
       <el-col :span="6">
          <el-button type="primary" @click="addSeriesProduct">添加类型</el-button>
       </el-col>
      </el-row>
      ...
      <el-dialog title="添加类型" :visible.sync="addDialogVisible" width="50%" :before-close="handleClose">
          <el-form :model="addClassForm" :rules="addRules" ref="addClassFormRef" label-width="120px">  
			  ...
			  <el-form-item label="下加光" prop="addLightBelow">
			  ...
			  <el-form-item label="光度范围2" prop="photometric2">
			      <el-input v-model="addClassForm.photometric2"></el-input>
			  </el-form-item>
			  <el-form-item label="光度范围3" prop="photometric3">
	  ...
	  <!-- 修改对话框 -->
	  <el-dialog title="修改类型" :visible.sync="editDialogVisible" width="50%" >
	      <el-form :model="editClassForm" :rules="editRules" ref="editClassFormRef" label-width="120px">  
	          ...
	          <el-form-item label="所属系列" >
	              <el-select v-model="editClassForm.seriesName" placeholder="请选择"   @change="handleEditSeriesChange">
	                <el-option
	                  v-for="item in seriesOptions"
	                  :key="item.id"
	                  :label="item.name"
    ...
    data() {
      return {
       ...
       options: '',
        ...
        addRules: {
			seriesId: [{required: true, message: '请选择所属类型', trigger: 'blur' }],
			name: [{required: true, message: '请输入类型名称', trigger: 'blur' }],
			refractive:[{required: true, message: '请输入折射率', trigger: 'blur' }],
			abbe: [{required: true, message: '请输入阿贝数', trigger: 'blur' }],
			film: [{required: true, message: '请输入膜层', trigger: 'blur' }],
			covert:[{required: true, message: '请输入隐形标记', trigger: 'blur' }],
			addLightBelow:[{required: true, message: '请输入下加光', trigger: 'blur' }],
			presentPrice:[{required: true, message: '请输入现片价', trigger: 'blur' }],
			customPrice: [{required: true, message: '请输入定制价', trigger: 'blur' }],
			photometric1: [{required: true, message: '请输入光度范围1', trigger: 'blur' }],
			photometric2: [{required: true, message: '请输入光度范围2', trigger: 'blur' }],
			photometric3: [{required: true, message: '请输入光度范围3', trigger: 'blur' }]
        },
		editClassForm: {
		   seriesId: '',
		   name: '',
		 ...
		 submitEdit() {
			this.$refs.editClassFormRef.validate((valid) => {
			  ...
			  if(!this.editSeriesId) {
			   ...
			   param.append('film',this.editClassForm.film);
